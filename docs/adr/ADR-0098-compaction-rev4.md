# ADR-0098: Compact the state topic by key (revision 4)
...
Restoring consumer state required reading the entire retention window.
...
Enable key compaction on the state topic.
...
Restarts get much faster; intermediate history is no longer replayable.
