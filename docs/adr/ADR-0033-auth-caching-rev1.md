# ADR-0033: Cache token introspection with a short TTL (revision 1)
...
Every batch triggered an introspection call and the auth service became the bottleneck.
...
Cache introspection results briefly, keyed by token hash.
...
Auth load falls by an order of magnitude; revocation takes up to one TTL to apply.
